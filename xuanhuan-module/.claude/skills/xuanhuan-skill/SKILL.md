---
name: xuanhuan-skill
description: 玄幻小说创作技能包。执行大纲、人物、目录、章节的专业创作，基于玄幻创作法则和史诗化的写作风格生成高质量玄幻小说内容。
---

# 玄幻小说创作 Skill

[技能说明]
    专业的玄幻小说创作技能包，覆盖世界观构建、人物塑造、修炼体系、章节规划、正文写作全流程。根据不同创作阶段，读取对应的创作资源并生成符合玄幻规范的小说内容。

[文件结构]
    .claude/skills/xuanhuan-skill/
    ├── SKILL.md                        # 本文件（技能包核心配置）
    ├── outline-method.md               # 大纲创作方法论（含世界观构建指导）
    ├── output-style.md                # 写作风格（含修炼体系和章节创作方法）
    ├── examples/
...
[核心能力]
    ...
    - **专业创作**：基于资源和上下文创作符合玄幻规范的小说内容
    - **风格把控**：确保创作内容符合玄幻的史诗化写作风格
    - **模板遵循**：严格按照模板格式生成文档结构
...
[执行流程]
    ...
    第二步：读取创作资源
        **大纲创作阶段**：
            ...
            2. 读取 outline-method.md（玄幻大纲创作方法论，含世界观构建指导）
            3. 读取 output-style.md（玄幻写作风格）
            4. 读取 examples/outline-example.md（玄幻大纲示例）
            5. 从对话历史获取用户回答的Q1-Q3（核心创意、核心冲突、故事调性）

        **人物创作阶段**：
            1. 读取 outline.md（了解故事背景和设定）
            2. 读取 templates/character-template.md（文档格式模板）
            3. 读取 outline-method.md（玄幻人物塑造指导部分）
            4. 读取 output-style.md（玄幻写作风格）
            5. 读取 examples/character-example.md（玄幻人物示例）

        **目录创作阶段**：
            1. 读取 outline.md 和 character.md（了解故事和人物）
            2. 读取 templates/chapter-index-template.md（文档格式模板）
            3. 读取 outline-method.md（玄幻章节规划方法）
            4. 读取 output-style.md（玄幻写作风格）

        **章节创作阶段**：
            1. 读取 outline.md、character.md、chapter_index.md（全部上下文）
            2. 读取 templates/chapter-template.md（文档格式模板）
            3. 读取 output-style.md（玄幻写作风格和章节创作方法，最重要）
            4. 读取 examples/chapter-example.md（玄幻章节示例）
    ...
    第三步：执行专业创作
        **大纲创作**：
            ...
            - 遵循 outline-method.md 的玄幻创作方法论
            - 应用 output-style.md 的史诗化写作风格
            - 参考 examples/outline-example.md 的示例
            - 生成包含以下内容的完整大纲：
                • 小说信息（书名、类型：玄幻、故事调性）
                • 核心梗概（一句话）
                • 导语（150-200字，包含核心冲突和世界观钩子）
        ...
        **人物创作**：
            ...
            - 遵循 outline-method.md 中的玄幻人物塑造指导
            - 应用 output-style.md 的史诗化写作风格
            - 参考 examples/character-example.md 的示例
            - 生成包含以下内容的人物小传：
                • 主角团（3-5个，详细描述，强调性格特点、修炼天赋和成长轨迹）
                • 反派角色（如有，包含实力和动机）
                • 重要配角（如有）
                • 其他角色（如有）
            - 确保人物性格鲜明，修炼体系清晰
        ...
        **目录创作**：
            ...
            - 遵循 outline-method.md 的玄幻章节规划方法
            - 应用 output-style.md 的写作风格
            - 生成固定70章的章节目录，与大纲的起承转合结构对应
            - 每章包含：章节标题 + 一句话剧情简介（体现修炼成长）
            - 确保起承转合布局合理，修炼节奏流畅

        **章节创作**：
            基于全部上下文文档和读取的资源：
            - 严格按照 templates/chapter-template.md 的格式结构
            - 严格遵循 output-style.md 的史诗化写作风格和章节创作方法（最重要）
...
[创作原则]
    ...
    - **质量标准原则**：
        • 大纲：核心冲突清晰，世界观完整，修炼体系合理，故事结构完整，固定起承转合结构，2000-3000字
        • 人物：性格鲜明，修炼天赋清晰，成长轨迹合理，与故事匹配
        • 目录：固定70章，与大纲的起承转合结构对应，布局合理，修炼推进流畅
        • 章节：2000-3000字，风格统一，战斗场面宏大，修炼体系清晰，推进剧情

[注意事项]
    - 确保每个阶段的必需资源都已读取完整
    - 不仅读取文档内容，还要深入理解其含义
    - output-style.md中的史诗化风格要求是强约束，必须严格遵守
    - templates/中的格式是必须遵循的，任何偏离都可能导致问题
    - 短篇小说固定为70章结构，采用起承转合布局
    - 创作内容必须完整、连贯、符合玄幻小说特点
    - 始终使用中文创作